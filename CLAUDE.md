...
- Run log: `coaching.lesson_runs` plus `coaching.lesson_run_notes`, and the same shape for games in `coaching.game_runs` plus `coaching.game_run_notes`. Only `user_profiles.is_coach` accounts can read or write them (`coaching.is_coach()`). A run stores the date, the group, an optional 1–5 score and a comment other coaches can read. One row per coach, item, date and group. Private notes are a separate table because row-level security cannot hide a single column. Lessons are logged from the lesson page. Warm-ups and mini games are logged from the game page, not from the lesson. The library shows that coach's last run and the average score.
