...
- Run log: `coaching.lesson_runs` plus `coaching.lesson_run_notes`. Only `user_profiles.is_coach` accounts can read or write runs (`coaching.is_coach()`). A run stores the date, the group, drill or coaching, an optional 1–5 score and a comment other coaches can read. One row per coach, lesson, date and group. Private notes are a separate table because row-level security cannot hide a single column. The lesson page logs and lists feedback. The library shows that coach's last run (narrowed by the group filter) and the average score. Both are hidden when printing. Logging a lesson can also rate the warm-up and mini game in that hour. Those rows live in `coaching.game_runs` (effectiveness and popularity, 1–5, shared with other coaches). The same form is on a warm-up or mini game page. The games library shows last run, effectiveness and popularity.
